...
class AutoCheckCodeQualityPlugin(BasePlugin):
    ...
    def should_run(self, event: Dict[str, Any]) -> bool:
        return event.get("type") == "pr" or "pull_request" in event
    ...
    def run(self, event: Dict[str, Any], context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for file_data in files_data:
            ...
            if "console.log" in patch:
                findings.append(f"{filename}: remove console.log before production.")
    ...
    async def execute(self, context: Dict[str, Any]) -> PluginResult:
        ...
        return PluginResult(
            success=True,
            action="code_review",
            message=f"Found {len(findings)} issues" if findings else "No issues found",
